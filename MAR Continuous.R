...
searlog(2, 0.5, m, rho_c, rho_o, delta, nc, tau_m=tau, B=1000)
semimonte(2, nc, pi, m, rho_c, rho_o, delta, tau, 0.5, B=1000)
...
setwd("~/Desktop/Fan Li Project/Project 1/Paper Material/Simulation results")
marfi <- read.csv("MAR Binary Final.csv")
marfi <- marfi[,-c(1)]
empirical.typeI.mar <- c()
empirical.power.mar <- c()
for (i in 1:144){
  simularesul <- simula(marfi[i,7], marfi[i,5], marfi[i,4], 
                                    marfi[i,3], marfi[i,2], marfi[i,10], 
                                    0.5, marfi[i,8], simt = 3000)
  empirical.typeI.mar[i] <- simularesul[1]
  empirical.power.mar[i] <- simularesul[2]
}


marfi$empirical.power <- empirical.power.mar
marfi$empirical.typeI.error <- empirical.typeI.mar
marfi$predicted.power.b <- round(marfi$predicted.power.b,3)
write.csv(marfi,"~/Desktop/Fan Li Project/Project 1/Paper Material/Simulation results/MAR BINARY FINAL.csv")
